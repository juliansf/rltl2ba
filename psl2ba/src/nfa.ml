...
(** Defines the type of an NFA. The transition function is defined as
    delta: states --> 2^(bexp x states) *)
...
(** Renumbers the states of a transition graph of an NFA by
    increasing the states by k.  Side effect: this procedure changes
...
let rem_unreachables (aut:nfa) : nfa =
  ...
  (* Construct new NFA *)
  let n' = Array.fold_left (fun i b ->
    if b then i+1 else i) 0 reachable in  (* Count reachable states *)
...
let concat (nfa1:nfa) (nfa2:nfa) : nfa =
  ...
  let final =
    if nfa2.final.(nfa2.start) then
      Array.append nfa1.final nfa2.final
    else
      Array.append (Array.make m1 false) nfa2.final
  in
...
let product (nfa1:nfa) (nfa2:nfa) : nfa =
  ...
  let rec dfs (q1:int) (q2:int) =
    ...
    delta := tr :: !delta;
     (* = [[succs of q_n]; [succs of q_{n-1}]; ...; [succs of q_0] *)
