...
struct
  ...
  let dnot x = Not x
  let dor x y = Or (x,y)
  let dand x y = And (x,y)

  let rec is_false = function
    | T -> false
    | F -> true
    | V _ | Not (V _)-> false
    | Not x -> not (is_false x)
  ...
  let rec print fmt = function
    ...
    | Not x -> Format.fprintf fmt "~%a" print x
    | And (x,y) -> Format.fprintf fmt "@[(%a & %a)@]" print x print y
    | Or (x,y) -> Format.fprintf fmt "@[(%a | %a)@]" print x print y
end
...
let normalize succ =
  let rec norm lst =
    match lst with
    | [] -> []
    | (l1,q1)::(l2,q2)::lst when q1=q2 -> norm ((Bdd.dor l1 l2, q1)::lst)
    | (l,_)::lst when Bdd.is_false l -> lst
    | x::lst -> x :: norm lst
  in
...
let merge_states nfa =
  ...
  let size = Array.length nfa.nfa_delta in

  (* Merge final/non-final states *)
  let merge nfa final_flag =
    ...
    let rename succ =
      List.map (fun (b,i) ->
        if Map.mem state_map i
        then (b, Map.find state_map i)
        else (b,i))
        succ
    in
  ...
  in
  let nfa = merge nfa true in
  let nfa = merge nfa false in
  nfa
...
(** Builds the NFA of a letter (Bdd.t) *)
let letter b =
  if Bdd.is_false b then _false
  else
    {
      nfa_delta = [|[(b,1)];[]|];
      nfa_start = 0;
...
(*
let rec succ_merge xs ys =
  match xs,ys with
  | [],l | l,[] -> l
  | (b,i)::xs', (c,k)::ys' ->
    if i<k then (b,i) :: succ_merge xs' ys
    else if i=k then (Bdd.dor b c, i) :: succ_merge xs' ys'
    else (c,k) :: succ_merge xs ys'
*)
...
let star2 nfa =
  if nfa=_false then _false else begin
    ...
    delta'.(n) <- start_succ;
    { nfa_delta=delta'; nfa_start=(size nfa); nfa_final=final'; }
  end

let star nfa = simplify (star2 nfa)
  ...
  if nfa1=_false || nfa2=_false then _false else begin
    ...
    Array.iteri (fun i succ ->
      List.iter (fun (b,i') ->
        if final1.(i') then
          List.iter (fun (c,k) ->
            delta.(i) <- (Bdd.dand b c,k)::succ) delta2.(start2)
      ) succ
    ) delta1;
  ...
  if nfa1=_false || nfa2=_false then _false else begin
    ...
    for i=0 to n1-1 do
      for j=0 to n2-1 do
        ...
        List.iter (fun (b,i') ->
          List.iter (fun (c,j') ->
            delta.(k) <- (Bdd.dand b c, i'*n2+j')::delta.(k)
          ) delta2.(j)) delta1.(i)
      done;
    done;
    simplify {
...
let printf fmt nfa =
  ...
  let rec print_delta fmt delta =
    ...
    | [] -> ()
    | (b,i)::delta' ->
      Format.fprintf fmt "@[%a -> %d@\n%a@]" Bdd.print b i print_delta delta'
  in
  for i=0 to n-1 do
...
let print nfa = printf Format.std_formatter nfa
...
open Bdd
*)
let c = letter (Bdd.V "c")
let a = letter (Bdd.V "a")
let b = letter (Bdd.V "b")
let d = letter (Bdd.V "d")
...
let nfa2 = {
  nfa_delta = [|[(c,1);(d,2)];[(dor a b,3);(a,3)]; [(b,3)]; []|];
  nfa_start = 0;
  nfa_final=[|false;false;false;true|]
}
let nfa3 = {
  nfa_delta = [|[(c,1);(d,2)];[(dor a b,3);(a,3)]; [(F,3)]; []|];
  nfa_start = 0;
  nfa_final=[|false;false;false;true|]
}
let nfa4 = {
  nfa_delta = [|[(c,1)];[(c,1)];[(a,2)]|];
  nfa_start = 0;
  nfa_final=[|false;false;true|]
}
let nfa5 = {
  nfa_delta = [|[(a,1);(b,3)];[(c,0)];[(a,4)];[(d,5)];[(b,2)];[]|];
  nfa_start = 0;
  nfa_final=[|false;false;false;true;false;false|]
}
