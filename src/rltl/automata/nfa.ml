...
let normalize succ =
  let rec norm lst =
    match lst with
    | [] -> []
    | (l1,q1)::(l2,q2)::lst when q1=q2 -> norm ((Cudd.dor l1 l2, q1)::lst)
    | (l,_)::lst when Cudd.is_false l -> lst
    | x::lst -> x :: norm lst
  in
...
let letter b =
  if Cudd.is_false b then _false
  else
    {
      nfa_delta = [|[(b,1)];[]|];
      nfa_start = 0;
...
let rec succ_merge xs ys =
  ...
  | (b,i)::xs', (c,k)::ys' ->
    ...
    else if i=k then (Cudd.dor b c, i) :: succ_merge xs' ys'
  ...
  if nfa1=_false || nfa2=_false then _false else begin
    ...
    Array.iteri (fun i succ ->
      List.iter (fun (b,i') ->
        if final1.(i') then
          List.iter (fun (c,k) ->
            delta.(i) <- (Cudd.dand b c,k)::succ) delta2.(start2)
      ) succ
    ) delta1;
  ...
  if nfa1=_false || nfa2=_false then _false else begin
    ...
    for i=0 to n1-1 do
      for j=0 to n2-1 do
        ...
        List.iter (fun (b,i') ->
          List.iter (fun (c,j') ->
            delta.(k) <- (Cudd.dand b c, i'*n2+j')::delta.(k)
          ) delta2.(j)) delta1.(i)
      done;
    done;
    simplify {
